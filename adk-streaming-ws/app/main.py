...
from loan_recovery_agent.agent import root_agent as loan_recovery_agent

# Select which agent to use - change this to switch between agents
# Options: "loan_recovery" or "google_search"
ACTIVE_AGENT = os.getenv("ACTIVE_AGENT", "loan_recovery")

# Set the root agent based on configuration
if ACTIVE_AGENT == "loan_recovery":
    root_agent = loan_recovery_agent
    print(f"🎯 Using Loan Recovery Agent for bidirectional streaming")
elif ACTIVE_AGENT == "google_search":
    root_agent = google_search_agent
    print(f"🎯 Using Google Search Agent for bidirectional streaming")
else:
    raise ValueError(f"Invalid ACTIVE_AGENT: {ACTIVE_AGENT}. Must be 'loan_recovery' or 'google_search'")
